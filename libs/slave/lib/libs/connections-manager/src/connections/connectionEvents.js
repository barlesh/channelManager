...
(function (connectionEvents) {
    function registerConnectionEvent(connection, protocolAction) {
    ...
    connectionEvents.registerConnectionEvent = registerConnectionEvent;
    async function runExec(event, exec, data) {
        const res = {
            status: false,
            data: undefined
        };
    ...
    async function publishConnectionEvent(connection, protocolAction, data) {
        if (!connection) {
            console.warn("no connection supplied. cannot send event");
        }
        const expectResponse = protocolAction.expectResponse;
        if (expectResponse) {
            const res = await publishConnectionEventAndListenToResponse(connection, protocolAction, data);
            return res;
        }
        else {
    ...
    connectionEvents.publishConnectionEvent = publishConnectionEvent;
    async function publishConnectionEventNoResponse(connection, data) {
        connection.emit(event, data);
    }
    async function publishConnectionEventAndListenToResponse(connection, request, data) {
